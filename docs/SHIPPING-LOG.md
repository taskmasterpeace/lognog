...
**New**
- **Dashboard Studio** — build dashboards by searching: run a query, preview it live, let LogNog suggest the best chart, drop panels onto a canvas, and save. Add a panel from any field in one click, and edit existing dashboards visually (the **Studio** button on any dashboard). Easier than Splunk's dashboard editor — no config files.
