...
export function PaginatedTable({
  ...
  return (
    <div className="flex flex-col h-full">
            ...
            <tr>
              {columns.map((column) => (
                <th
                  ...
                  className="text-left p-2 border-b border-nog-200 dark:border-nog-700 font-medium text-nog-600 dark:text-nog-400 cursor-pointer hover:bg-nog-50 dark:hover:bg-nog-700/50 select-none"
                >
                  <div className="flex items-center gap-1">
                    <span>{column}</span>
                    {sortColumn === column && (
                      sortDirection === 'asc' ? (
                        <ArrowUp className="w-3.5 h-3.5 text-honey-500" />
                      ) : (
                        <ArrowDown className="w-3.5 h-3.5 text-honey-500" />
                      )
                    )}
                  </div>
                </th>
      ...
      {/* Pagination */}
      <div className="flex items-center justify-between px-2 py-2 border-t border-nog-200 dark:border-nog-700 bg-nog-50 dark:bg-nog-800/50">
        <div className="flex items-center gap-2 text-sm text-nog-600 dark:text-nog-400">
          <span>Show</span>
          <select
            value={pageSize}
            onChange={(e) => handlePageSizeChange(Number(e.target.value))}
            className="px-2 py-1 border border-nog-200 dark:border-nog-600 rounded bg-white dark:bg-nog-700 text-sm"
          >
        ...
        <div className="text-sm text-nog-600 dark:text-nog-400">
        ...
        <div className="flex items-center gap-1">
          <button
            onClick={() => setPage(0)}
            disabled={page === 0}
            className="p-1.5 rounded hover:bg-nog-200 dark:hover:bg-nog-700 disabled:opacity-50 disabled:cursor-not-allowed"
            title="First page"
          >
          ...
          <div className="flex items-center gap-1 mx-2">
            {generatePageNumbers(page, totalPages).map((p, i) =>
              p === '...' ? (
                <span key={`ellipsis-${i}`} className="px-2 text-nog-400">
                  ...
                </span>
              ) : (
                <button
                  ...
                  className={`px-2.5 py-1 rounded text-sm ${
                    page === p
                      ...
                      : 'hover:bg-nog-200 dark:hover:bg-nog-700'
                  }`}
                >
                  {(p as number) + 1}
                </button>
