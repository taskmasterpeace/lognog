  ...
  return (
    <div className="max-w-xs">
      ...
      {annotation.description && (
        <p className="text-sm text-nog-300 mb-2">{annotation.description}</p>
      )}
      {annotation.lookupData && Object.keys(annotation.lookupData).length > 0 && (
        <div className="text-xs border-t border-nog-600 pt-2 mt-2">
          {Object.entries(annotation.lookupData).slice(0, 3).map(([key, val]) => (
            <div key={key} className="flex justify-between gap-2">
              <span className="text-nog-400">{key}:</span>
              <span className="text-nog-200 truncate">{String(val)}</span>
            </div>
          ))}
        </div>
      )}
      {annotation.details && onClick && (
        <button
          onClick={onClick}
          className="text-xs text-honey-400 hover:text-honey-300 mt-2 flex items-center gap-1"
        >
    ...
    >
      <span
        className={`
          ...
          ${annotation.color ? '' : 'border-b border-dotted border-nog-400 dark:border-nog-500'}
        `}
      ...
      >
        ...
        {children}
        <Info className="inline-block w-3 h-3 ml-1 text-nog-400 opacity-0 group-hover:opacity-100 transition-opacity" />
      </span>
    </Tooltip>
  );
}
  ...
  return (
    <>
      ...
      <div className="fixed right-0 top-0 h-full w-96 bg-white dark:bg-nog-800 shadow-2xl z-50 animate-slide-in-right overflow-hidden flex flex-col">
        {/* Header */}
        <header className="flex items-center justify-between p-4 border-b border-nog-200 dark:border-nog-700">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            {onEdit && (
              <button
                onClick={onEdit}
                className="p-2 text-nog-500 hover:text-nog-700 dark:text-nog-400 dark:hover:text-nog-200 hover:bg-nog-100 dark:hover:bg-nog-700 rounded-lg transition-colors"
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          {annotation.details && (
            <div>
              <h3 className="text-sm font-medium text-nog-500 dark:text-nog-400 mb-2">
                Details
              </h3>
              <div className="prose prose-sm dark:prose-invert max-w-none bg-nog-50 dark:bg-nog-900 rounded-lg p-3">
                {/* Simple markdown rendering - just paragraphs and line breaks */}
                {annotation.details.split('\n\n').map((paragraph, i) => (
                  <p key={i} className="mb-2 last:mb-0 text-nog-700 dark:text-nog-300">
                    {paragraph.split('\n').map((line, j) => (
          ...
          {annotation.lookupData && Object.keys(annotation.lookupData).length > 0 && (
            <div>
              ...
              <div className="bg-nog-50 dark:bg-nog-900 rounded-lg overflow-hidden">
                <table className="w-full text-sm">
                  <tbody>
                    {Object.entries(annotation.lookupData).map(([key, val]) => (
                      <tr key={key} className="border-b border-nog-200 dark:border-nog-700 last:border-0">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {annotation.tags.map((tag) => (
                  <span
                    key={tag}
                    className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-honey-100 text-honey-800 dark:bg-honey-900/30 dark:text-honey-300"
                  >
                    {tag}
                  </span>
                ))}
        ...
        {/* Footer */}
        <footer className="p-4 border-t border-nog-200 dark:border-nog-700 text-xs text-nog-500 dark:text-nog-400">
